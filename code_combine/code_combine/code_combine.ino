...
void setup() {
  ...
  if (t.err) {
    display.clearBuffer();
    display.drawStr(0, 0, "WiFi connected!");
    display.sendBuffer();
    delay(1000);  // Hiển thị trạng thái thành công trong 1s
  } else {
    display.clearBuffer();
    display.drawStr(0, 0, "WiFi failed!");
    display.sendBuffer();
    delay(1000);
    ESP.restart();
  }
...
const unsigned long interval = 1000;  // Khoảng thời gian giữa các lần đọc cảm biến
